...
# Dyanmically set your input file based on the last argument in your slurm submission (ie. your CSV):
input = commandArgs(trailingOnly = TRUE)[1]

# Extract module name
module_name <- sub("_numeric.csv", "", basename(input))
module_name <- sub("^merged_hostGO_MM_MWU_", "", module_name)


# Set directory and file constants:
setwd(file.path("/scratch1/migomez/CalcExpt/GO_MWU_Deepsplit2", module_name))
input <- basename(input)

goAnnotations="annot_go_clean_final.tab" # two-column, tab-delimited, one line per gene, multiple GO terms separated by semicolon. If you have multiple lines per gene, use nrify_GOtable.pl prior to running this script.
goDatabase="go.obo" # download from http://www.geneontology.org/GO.downloads.ontology.shtml
goDivision="CC" # either MF, or BP, or CC (molecular function, biological function, cellular components)
